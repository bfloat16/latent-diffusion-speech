...
def sinusoids(length, channels, max_timescale=10000):
    ...
    return torch.cat([torch.sin(scaled_time), torch.cos(scaled_time)], dim=1).to(device="cuda")
...
class AudioEncoder(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        x = x.permute(0, 2, 1)
        x = (x + sinusoids(x.size(1), self.n_audio_state)).to(x.dtype)
...
class Whisper(nn.Module):
    def __init__(self, dims: ModelDimensions):
        super().__init__()
        self.dims = dims
        self.encoder = AudioEncoder(self.dims.n_mels, self.dims.n_audio_state, self.dims.n_audio_head, self.dims.n_audio_layer)
